...
public class PointsController {
    ...
    @GetMapping("/mall")
    public ApiResponse<List<MallItem>> listMallItems() {
        String userId = currentUserUtil.getCurrentUserId();
        if (userId == null) {
            return ApiResponse.error("请先登录");
        }
    ...
    public ApiResponse<Coupon> redeemMallItem(@PathVariable String itemId) {
        String userId = currentUserUtil.getCurrentUserId();
        if (userId == null) {
            return ApiResponse.error("请先登录");
        }
    ...
    @GetMapping("/coupons")
    public ApiResponse<Page<Coupon>> myCoupons(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        String userId = currentUserUtil.getCurrentUserId();
        if (userId == null) {
        ...
        return ApiResponse.success(mallService.listMyCoupons(userId, page, size));
    }
}
